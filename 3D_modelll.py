...
def calculate_h_i(chi_past_h_i, chi_next_e_k, chi_previous_e_k, chi_next_e_j, chi_previous_e_j, chi_j_step, chi_k_step,
    ...
    chi_c = calculate_C_coefficient(chi_permeability, chi_time_step, chi_magnetic_conductivity)
    chi_d = calculate_D_coefficiend(chi_permeability, chi_time_step, chi_magnetic_conductivity)
    chi_h_i = chi_c*chi_past_h_i - chi_d*((chi_next_e_k - chi_previous_e_k)/chi_j_step - (chi_next_e_j - chi_previous_e_j)/chi_k_step - chi_magnetic_current_x)
    return chi_h_i
...
def calculate_e_i(cei_past_e_i, cei_next_h_k, cei_previous_h_k, cei_nex_h_j, cei_previous_h_j, cei_j_step,
    ...
    cei_q = calculate_Q_coefficient(cei_permittivity, cei_time_step, cei_electrical_conductivity)
    cei_f = calculate_F_coefficient(cei_permittivity, cei_time_step, cei_electrical_conductivity)
    cei_e_i = cei_q*cei_past_e_i + cei_f*((cei_next_h_k - cei_previous_h_k)/cei_j_step -
                                                                                  (cei_nex_h_j - cei_previous_h_j)/cei_k_step - cei_electrical_current)
    return cei_e_i
...
def calculate_h_i(chi_past_h_i, chi_next_e_k, chi_previous_e_k, chi_next_e_j, chi_previous_e_j, chi_j_step, chi_k_step,
                  chi_time_step, chi_permeability, chi_magnetic_conductivity, chi_magnetic_current_x):
    Z = 376.7
    chi_h_i = chi_past_h_i - 1/Z*chi_time_step*((chi_next_e_k - chi_previous_e_k)/chi_j_step - (chi_next_e_j - chi_previous_e_j)/chi_k_step)
    return chi_h_i
...
def calculate_e_i(cei_past_e_i, cei_next_h_k, cei_previous_h_k, cei_nex_h_j, cei_previous_h_j, cei_j_step,
    ...
    cei_e_i = cei_past_e_i + Z*cei_time_step*((cei_next_h_k - cei_previous_h_k)/cei_j_step -
                                                                                  (cei_nex_h_j - cei_previous_h_j)/cei_k_step)
    return cei_e_i
'''
def calculate_h_i(chi_past_h_i, chi_next_e_k, chi_previous_e_k, chi_next_e_j, chi_previous_e_j, chi_j_step, chi_k_step,
                  chi_time_step, chi_permeability, chi_magnetic_conductivity, chi_magnetic_current_x):
...
'''
for n in range(1,time_point_number-1):
    for i in range(1,x_point_number-1):
        for j in range(1, y_point_number-1):
            for k in range(1, z_point_number-1):
                magnetic_field_x[1][i][j][k] = calculate_h_i(magnetic_field_x[0][i][j][k], electric_field_z[0][i][j][k],
                                                          electric_field_z[0][i][j-1][k],
                                                          electric_field_y[0][i][j][k],
                                                          electric_field_y[0][i][j][k-1],  y_step, z_step, tau_step,
                                                          1, 0, 0)
                magnetic_field_y[1][i][j][k] = calculate_h_i(magnetic_field_y[0][i][j][k], electric_field_x[0][i][j][k],
                                                          electric_field_x[0][i][j][k-1],
                                                          electric_field_z[0][i][j][k],
                                                          electric_field_z[0][i-1][j][k],  z_step, x_step, tau_step,
                                                          1, 0, 0)
                magnetic_field_z[1][i][j][k] = calculate_h_i(magnetic_field_z[0][i][j][k], electric_field_y[0][i][j][k],
                                                          electric_field_y[0][i-1][j][k],
                                                          electric_field_x[0][i][j][k],
                                                          electric_field_x[0][i][j-1][k],  x_step, y_step, tau_step,
                                                          1, 0, 0)
                electric_field_x[1][i][j][k] = calculate_e_i(electric_field_x[0][i][j][k], magnetic_field_z[1][i][j+1][k],
                                                          magnetic_field_z[1][i][j][k],
                                                          magnetic_field_y[1][i][j][k+1],
                                                          magnetic_field_y[1][i][j][k],  y_step, z_step, tau_step,
                                                          1, 0, 0)
                electric_field_y[1][i][j][k] = calculate_e_i(electric_field_y[0][i][j][k], magnetic_field_x[1][i][j][k+1],
                                                          magnetic_field_x[1][i][j][k],
                                                          magnetic_field_z[1][i+1][j][k],
                                                          magnetic_field_z[1][i][j][k],  z_step, x_step, tau_step,
                                                          1, 0, 0)
                electric_field_z[1][i][j][k] = calculate_h_i(electric_field_z[0][i][j][k], magnetic_field_y[1][i+1][j][k],
                                                          magnetic_field_y[1][i][j][k],
                                                          magnetic_field_x[1][i][j+1][k],
                                                          magnetic_field_x[1][i][j][k],  x_step, y_step, tau_step,
                                                          1, 0, 0)
    ...
    e_x_result[n - 1] = electric_field_y[0][result_x][result_y][result_z]
    e_y_result[n - 1] = electric_field_y[0][result_x][result_y][result_z]
    e_z_result[n - 1] = electric_field_z[0][result_x][result_y][result_z]
    h_x_result[n - 1] = magnetic_field_y[0][result_x][result_y][result_z]
    h_y_result[n - 1] = magnetic_field_y[0][result_x][result_y][result_z]
    h_z_result[n - 1] = magnetic_field_z[0][result_x][result_y][result_z]
    ...
    if n % 1 == 0:
        ...
        Intencity = (electric_field_x[0][int(x_point_number / 2)][int(y_point_number / 2)] ** 2 +
                     ...
                     electric_field_z[0][int(x_point_number / 2)][int(y_point_number / 2)] ** 2) ** 0.5

        z = np.arange(0, z_point_number)
        plt.ioff()
        fig, ax = plt.subplots()

        ax.plot(z, electric_field_x[0][x_point_number-15][y_point_number-15], 'rx', linestyle='solid')
        #ax.plot(z, Intencity, 'rx', linestyle='solid')
        #save('3D_mod' + str(n), format='png')
        plt.show()
        plt.savefig('3D_mod' + str(n))
'''
...
t = np.arange(0, time_point_number)
plt.ioff()
fig, ax = plt.subplots()

ax.plot(t, e_x_result, 'rx', linestyle='solid')
plt.show()
